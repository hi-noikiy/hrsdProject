...
function getConfig(opts = {}) {
  ...
  if ((0, _fs.existsSync)((0, _path.resolve)('.eslintrc'))) {
    try {
      const userRc = (0, _readRc.default)((0, _path.resolve)('.eslintrc'));
      debug(`userRc: ${JSON.stringify(userRc)}`);

      if (userRc.extends) {
        debug(`use user's .eslintrc: ${(0, _path.resolve)('.eslintrc')}`);
        eslintOptions.useEslintrc = true;
        eslintOptions.baseConfig = false;
        eslintOptions.ignore = true;
      } else {
        debug(`extend with user's .eslintrc: ${(0, _path.resolve)('.eslintrc')}`);
  ...
  const config = {
    ...
    resolve: {
      ...
      alias: _objectSpread({
        '@babel/runtime': (0, _path.dirname)(require.resolve('@babel/runtime/package.json'))
      }, opts.alias),
